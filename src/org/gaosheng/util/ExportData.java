...
import org.gaosheng.util.database.Connect;

public class ExportData {

	@SuppressWarnings("unchecked")
	public static void main(String[] args) {
		Connection conn = Connect.getNew_Conn();
		...
		try {
			fileOut = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(new File("E:/update.sql"))));
			...
			while (rs.next()){
				...
				fileOut.write(String.format(updateModel, rs.getString(2),rs.getString(3),rs.getString(4),rs.getString(5),rs.getString(6),rs.getString(7),rs.getString(8),rs.getString(9),rs.getString(10),rs.getString(1)));
			}
			fileOut.flush();
		} catch (Exception e1) {
			e1.printStackTrace();
