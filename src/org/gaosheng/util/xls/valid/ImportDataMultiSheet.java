package org.gaosheng.util.xls.valid;
			...
			for (int sheet_index = 0; sheet_index < sheet_num ;sheet_index++) {
				...
				for (int i = 0; i < startRow; i++) {
					...
					for (int j = 0; j < columns; j++) {
						...
						if (viewWidthList.size() <= j || viewWidthList.get(j) < c_temp.length()) {
...
//					System.out.println();
					if (info_temp.length() > 1) {
						errors.add("sheet "+sheet.getName()+" 中第 " + (i + 1) + " 行 :"+ info_temp.toString());
						for (int ii = 0; ii < columns; ii++) {
							String c_temp = getCellValue(sheet.getCell(ii, i));
							if (viewWidthList.size() <= ii || viewWidthList.get(ii) < c_temp.length()) {
						...
						if (viewWidthList.size() <= columns || viewWidthList.get(columns) < info_temp.toString().length()) {
							viewWidthList.add(columns, info_temp.toString().length());
						}
		...
		if (successcount < totalRows - sheet_num*startRow) {
			System.out.println("导入情况: "+successcount + "  "+ (totalRows - sheet_num*startRow));
			this.hasError = true;
		}
		return true;
	}
