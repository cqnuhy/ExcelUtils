...
public abstract class ImportData {
	 ...
	 */
	public boolean execute() throws SecurityException, NoSuchMethodException, EntityException{
		setMethodMap();
		setHoldColumns();
		Workbook work = null;
		try {
		...
		if (columnMethods == null) {
			...
			for (int i = 0; i < methods.length; i++) {
				if (methods[i].getName().startsWith("validColumn_")) {
					String column = methods[i].getName().substring(methods[i].getName().indexOf("_")+1);
					try {
